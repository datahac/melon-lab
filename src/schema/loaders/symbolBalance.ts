...
export const getTokenBalance = async (environment, deployment, symbol, address) => {
  const token = deployment.tokens.find(item => item.symbol === symbol);
  const quantity = await protocol.token.balanceOf(token.address, { address }, environment);
  return quantity;
};

export const getSymbolBalance = R.curryN(3, async (environment, deployment, symbol, address) => {
  const quantity = symbol === 'ETH' ?
    await getEthBalance(environment, address) :
    await getTokenBalance(environment, deployment, symbol, address);

  return extractQuantity(quantity);
});

export const observeSymbolBalance = R.curryN(3, (environment, streams, symbol, address) => {
  if (symbol === 'ETH') {
    const stream$ = streams.block$.pipe(switchMap(() => {
  ...
  const stream$ = streams.deployment$.pipe(switchMap((deployment) => {
    const token = deployment.tokens.find(item => item.symbol === symbol);
    const zen = protocol.token.balanceOf.observable(token.address, { address }, environment);
    return Rx.from(zen);
  }));
