...
import { debounceTime, switchMap, filter, tap, withLatestFrom } from 'rxjs/operators';
...
const WrappedOrderForm = withApollo(
  withForm(props => {
    useObservable(
        ...
        const kyber$ = inputs$.pipe(
          ...
          switchMap(async ([_, quantity, total, __, type]) => {
            const result = await props.client.query({
              ...
              variables: {
                quantity: (quantity && quantity.quantity.toString()) || '1000000000000000000',
                total: (total && total.quantity.toString()) || '1000000000000000000',
      ...
      [props.values.exchange, props.values.quantity, props.values.total, props.enginePrice, props.values.type],
    );

    const limitExchanges = props.exchanges
      .filter(([value]) => !R.includes(value, ['KYBER_NETWORK', 'MELON_ENGINE']))
      .map(([value, name]) => ({
        value,
        name,
      }));
...
const OrderFormContainer: React.PureComponent<{}, {}> = props => {
  ...
  return (
    <Composer components={[<NetworkConsumer />, <FundManagerConsumer />, <EngineQuery />]}>
      {([network, managerProps, engine]) => {
        const isManager = !!managerProps.fund && isSameAddress(managerProps.fund, props.address);
