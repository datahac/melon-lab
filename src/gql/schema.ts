...
const signWithPrivateKey = async (environment, wallet, unsigned) => {
  ...
  const signed = await accounts.signTransaction(unsigned, wallet && wallet.privateKey);
...
addQueryDirectives(schema, {
  ...
  sign: async (resolve, source, args, context, info, directiveArgs) => {
    ...
    const unsigned = R.pick(['data', 'from', 'gas', 'gasPrice', 'to', 'value'], args[directiveArgs.source]);
    ...
    // skip signing here.
    const signedOrNot =
      walletType === WalletTypes.HARDWARE ? unsigned : await signWithPrivateKey(environment, wallet, unsigned);
