...
import { managersToRoutes, isEmptyAddress } from '@melonproject/protocol';

async function routes(environment, managerAddress) {
  const routes = await managersToRoutes(environment, environment.deployment.melonContracts.version, managerAddress);
