...
export default (environment$, streams) => {
  ...
  const fundPolicies = new DataLoader(async addresses => {
    ...
    return Promise.all(
      addresses.map((address, key) => {
        ...
        return policyManagerAddress && getFundPolicies(env, policyManagerAddress);
      }),
    );
  });
  ...
  const fundDenominationAsset = new DataLoader(async addresses => {
    ...
    return Promise.all(
      addresses.map((address, key) => {
        ...
        return accountingAddress && getFundDenominationAsset(env, accountingAddress);
      }),
    );
  });
  ...
  const fundHoldings = new DataLoader(async addresses => {
    ...
    return Promise.all(
      addresses.map((address, key) => {
        ...
        return accountingAddress && tradingAddress && getFundHoldings(env, accountingAddress, tradingAddress);
      }),
    );
  });
    ...
    async pairs => {
      ...
      return Promise.all(
        investors.map((investor, key) => {
          ...
          return sharesAddress && getFundParticipation(env, sharesAddress, investor);
        }),
      );
    },
    {
  ...
  const exchangeOrders = new DataLoader(
    async pairs => {
      ...
      const result = pairs.map(pair => fn(pair.exchange, pair.base, pair.quote));
      const raw = await Promise.all(result || []);
      return raw;
    },
    {
      cacheKeyFn: options => `${options.exchange}:${options.base}:${options.quote}`,
    },
  );
