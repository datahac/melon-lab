...
const withHardwareSigner = (environment: Environment, account: Tm.Address) => {
  const signMessage = async message => {
    const signature = await environment.eth.personal.sign(message, account.toString());
...
const estimateMakeOrder = async (
  ...
  const makerQuantity = Tm.createQuantity(getTokenBySymbol(environment, sellToken), sellQuantity);
  const takerQuantity = Tm.createQuantity(getTokenBySymbol(environment, buyToken), buyQuantity);
  ...
  if (exchange === 'RADAR_RELAY') {
    const zeroExAddress = R.path(['deployment', 'exchangeConfigs', Exchanges.ZeroEx, 'exchange'], env);
    ...
    try {
      ...
      */
      const response = await axios.post(
        `https://api${networkName !== 'LIVE' ? '.kovan' : ''}.radarrelay.com/0x/v2/order_config`,
        payload,
      );
  ...
  if (exchange === 'ETHFINEX') {
    const wrapperRegistryEfx = R.path(
      ['deployment', 'thirdPartyContracts', 'exchanges', 'ethfinex', 'wrapperRegistryEFX'],
      env,
    );
    const ethfinexAddress = R.path(['deployment', 'exchangeConfigs', Exchanges.Ethfinex, 'exchange'], env);
    // TODO: Refactor this into a directive
    const wallet = loaders.getWallet();
    const walletType = loaders.getWalletType();
    ...
    const wrappedMakerQuantity = Tm.createQuantity(wrappedMakerToken, makerQuantity.quantity);
