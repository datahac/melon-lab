...
import { withDifferentAccount, beginSetup, getTokenByAddress } from '@melonproject/protocol';

const estimateFundSetupBegin = async (
  _,
  { from, name, exchanges, assets, managementFee, performanceFee, feePeriod = '90' },
  { loaders },
) => {
  ...
  const defaultTokens = assets.map(address => getTokenByAddress(environment, address)).filter(token => !!token);
