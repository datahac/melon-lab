...
const executeMakeOrder = async (_, { from, signedOrNot, exchange, signedOrder }, { loaders }) => {
  ...
  if (exchange === 'OASIS_DEX') {
    const result = await makeOasisDexOrder.send(env, tradingAddress, transaction);
  ...
  if (exchange === 'RADAR_RELAY') {
    ...
    try {
      ...
      if (chain === 'kovan') {
        await axios.post('https://api.kovan.radarrelay.com/v2/orders', parsedOrder);
  ...
  if (exchange === 'ETHFINEX') {
    ...
    try {
      const result = await makeEthfinexOrder.send(env, tradingAddress, transaction, {
        signedOrder: parsedOrder,
      });
