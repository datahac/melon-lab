...
import { withDifferentAccount, cancelOasisDexOrder, cancel0xOrder, cancelEthfinexOrder } from '@melonproject/protocol';

const executeCancelOrder = async (_, { from, signedOrNot, exchange }, { loaders }) => {
  ...
  if (exchange === 'OASIS_DEX') {
    const result = await cancelOasisDexOrder.send(env, tradingAddress, transaction);
  ...
  if (exchange === 'ETHFINEX') {
    const result = await cancelEthfinexOrder.send(env, tradingAddress, transaction);
