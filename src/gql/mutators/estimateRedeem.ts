...
import { getFundToken, withDifferentAccount, redeemQuantity } from '@melonproject/protocol';

const estimateRedeem = async (_, { from, fundAddress, sharesQuantity }, { loaders }) => {
  ...
  const result = await redeemQuantity.prepare(env, participationAddress.toString(), {
