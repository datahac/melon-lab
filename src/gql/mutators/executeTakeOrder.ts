...
const executeTakeOrder = async (_, { from, signedOrNot, exchange }, { loaders }) => {
  ...
  if (exchange === 'OASIS_DEX') {
    const result = await takeOasisDexOrder.send(env, tradingAddress, transaction);

    const type = Tm.isEqual(denominationAsset, result.sell.token) ? 'BUY' : 'SELL';

    const trade = type === 'BUY' ? Tm.createPrice(result.buy, result.sell) : Tm.createPrice(result.sell, result.buy);
  ...
  if (exchange === 'KYBER_NETWORK') {
    const result: TakeOrderOnKyberResult = await takeOrderOnKyber.send(env, tradingAddress, transaction);

    const type = Tm.isEqual(denominationAsset, result.takerQuantity.token) ? 'BUY' : 'SELL';
  ...
  if (exchange === 'RADAR_RELAY') {
    ...
    const type = Tm.isEqual(denominationAsset, result.takerFilledAmount.token) ? 'BUY' : 'SELL';
