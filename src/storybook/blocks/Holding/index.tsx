  ...
  return (
    <Fragment>
      ...
      <Row className={holdingClassNames} onClick={onClickHolding}>
          ...
          <span className="holding__symbol">
            ...
            {isManager && locked && !Tm.isZero(locked) && (
              <span className="holding__release">
                <Button style="danger" size="tiny" alt={Tm.toFixed(locked, decimals)} onClick={onClickRelease}>
                  Release
                </Button>
              </span>
            )}
        ...
        <CellBody noPadding={false}>
          <div className="holding__price">{price && Tm.toFixed(price, decimals)}</div>
        </CellBody>
        <CellBody noPadding={false}>
          <div className="holding__balance">{balance && Tm.toFixed(balance, decimals)}</div>
        </CellBody>
      </Row>
    </Fragment>
  );
