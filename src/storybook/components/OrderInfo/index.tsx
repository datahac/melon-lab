...
const OrderInfo: StatelessComponent<OrderInfoProps> = ({ ask, bid, lastPrice, baseToken, quoteToken }) => (
  <div className="order-info">
    <style jsx>{styles}</style>
    <div className="order-info__prices">
      <div className="order-info__last-price">
        {lastPrice ? (
          <Fragment>{lastPrice && Tm.isPrice(lastPrice) && Tm.toFixed(lastPrice)}</Fragment>
        ) : (
          <span>N/A</span>
        )}
    ...
    <div className="order-info__balances">
      <div className="order-info__balance">
        <span className="order-info__balance-desc">{R.path(['token', 'symbol'], baseToken)}:</span>{' '}
        {baseToken && Tm.isQuantity(baseToken) && Tm.toFixed(baseToken)}
      </div>
      <div className="order-info__balance">
        <span className="order-info__balance-desc">{R.path(['token', 'symbol'], quoteToken)}:</span>{' '}
