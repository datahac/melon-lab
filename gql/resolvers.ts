...
export default {
  ...
  Query: {
    ...
    fund: (_, { address }, { loaders }) => {
      return (
        (Tm.isAddress(address) &&
          loaders.fundReady
            .load(address)
            .then(R.cond([[R.equals(true), R.always(address)], [R.equals(false), R.always(null)]]))) ||
        null
      );
    },
    ...
    kyberPrice: async (_, { symbol, quantity, total, type }, { loaders }) => {
      ...
      const kyberNetworkProxy = R.path(
        ['deployment', 'thirdPartyContracts', 'exchanges', 'kyber', 'kyberNetworkProxy'],
        environment,
      );

      const weth = getTokenBySymbol(environment, 'WETH');
      const makerAsset = type === 'BUY' ? getTokenBySymbol(environment, symbol) : weth;
      const takerAsset = type === 'SELL' ? getTokenBySymbol(environment, symbol) : weth;

      const fillTakerQuantity = Tm.createQuantity(takerAsset, type === 'SELL' ? quantity : total);
      ...
      const price = type === 'BUY' ? Tm.normalize(Tm.createPrice(rate.quote, rate.base)) : rate;

      return price;
    },
    enginePrice: async (_, __, { loaders }) => {
      const environment = await loaders.environment();
      const engineAddress = R.path(['deployment', 'melonContracts', 'engine'], environment);

      const weth = getTokenBySymbol(environment, 'WETH');

      const enginePrice = await getEnginePrice(environment, engineAddress);

      const enginePriceWeth = Tm.createPrice(enginePrice.base, Tm.createQuantity(weth, enginePrice.quote.quantity));
    ...
    openOrders: async (_, { fundAddress }, { loaders }) => {
      const environment = await loaders.environment();
      const { tradingAddress, accountingAddress } = await loaders.fundRoutes.load(fundAddress);
      ...
      const result = openOrders.map(order => {
        const type = Tm.isEqual(denominationAsset, order.makerQuantity.token) ? 'BID' : 'ASK';
      ...
      // TODO: Remove thiss
      const withoutEthfinex = result.filter(order => order.exchange !== 'ETHFINEX');
  ...
  Mutation: {
    ...
    importWallet: (_, { wallet, password }, { loaders }) => {
      return null;
      // return loaders.importWallet(wallet, password, async (decrypted, encrypted) => {
    ...
    restoreWallet: (_, { mnemonic, password }, { loaders }) => {
      return null;
      // return loaders.restoreWallet(mnemonic, password, async (decrypted, encrypted) => {
      //   await keytar.setPassword('melon.fund', decrypted.address, encrypted);
      //   loaders.setWallet(decrypted);
      // });
    },
    ...
    useFrame: async (_, { address }, { loaders }) => {
      ...
      if (address.toLowerCase() !== ethAccounts[0].toLowerCase()) {
        throw new Error(`Address mismatch. ethAccounts: ${ethAccounts[0]}, argument: ${address}`);
      }
  ...
  Subscription: {
    ...
    priceFeedUp: {
      ...
      subscribe: (_, __, { streams }) => {
        const stream$ = streams.recentPrice$.pipe(distinctUntilChanged(R.equals));
