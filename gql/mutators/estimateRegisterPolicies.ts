...
import { withDifferentAccount, FunctionSignatures, register } from '@melonproject/protocol';
...
const estimateRegisterPolicies = async (_, { from, policies }, { loaders }) => {
  ...
  const result = await register.prepare(env, policyManagerAddress, registrations);
