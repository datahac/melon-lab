...
import { withdraw, withDifferentAccount, getTokenBySymbol } from '@melonproject/protocol';
...
const executeWithdraw = async (_, { from, signedOrNot }, { loaders }) => {
  ...
  const result = await withdraw.send(env, wethToken.address as Tm.Address, transaction);
