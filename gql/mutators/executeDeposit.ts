...
import { deposit, withDifferentAccount, getTokenBySymbol } from '@melonproject/protocol';
...
const executeDeposit = async (_, { from, signedOrNot }, { loaders }) => {
  ...
  const result = await deposit.send(env, wethToken.address as Tm.Address, transaction);
