...
const takeOrder = async (
  ...
  if (takerFee && new BigNumber(takerFee).gt(0)) {
    ...
    const zeroExBalance = await getBalance(environment, {
      tokenSymbol: zeroExTokenSymbol,
      ofAddress: fundAddress,
    });
    ensure(
      zeroExBalance.gte(new BigNumber(takerFee)),
      'You do not own enough ${zeroExTokenSymbol} token to cover for the relayer fee. Please buy ${zeroExTokenSymbol} tokens if you wish to take that order.',
    );
  }
