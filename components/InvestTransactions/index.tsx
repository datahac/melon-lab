...
import { estimateApproveTransferMutation, executeApproveTransferMutation } from '~/queries/approve.gql';
...
export default withRouter(props => {
  ...
  if ((props.isInitialRequest || !props.isWaiting) && !props.isExpired && props.values && props.values.total) {
    estimations.push(
      {
        mutation: props.step === 1 ? estimateApproveTransferMutation : estimateNothing,
        variables:
          props.step === 1
            ? {
      ...
      {
        mutation: props.step === 2 ? estimateRequestInvestmentMutation : estimateNothing,
        variables:
          props.step === 2
            ? {
                fundAddress: props.fundAddress,
  ...
  if (!props.isExpired && (props.isInitialRequest || props.readyToExecute)) {
    estimations.push({
