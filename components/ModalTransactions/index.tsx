    ...
    return (
      rendered && (
        ...
        >
          ...
          <TransactionProgress transactions={estimations} activeTransaction={step} />
...
const ModalTransactions = ({ executions, estimations, handleCancel, handleComplete, text, open }) => {
  const filteredEstimations = estimations.filter(mutation => !mutation.isComplete);
  ...
  return (
    (estimation && execution && (
      <Composer
        components={[
          ({ render }) => (
            <Mutation {...R.omit(['name', 'isComplete'], estimation)}>{(a, b) => render([a, b])}</Mutation>
          ),
          ...
          return (
            <Fragment>
              <WithFormModal
                ...
                current={estimation}
                step={!R.isEmpty(filteredEstimations) && filteredEstimations[0].name}
                gasLimit={R.path(['data', 'estimate', 'gas'], estimateProps)}
                gasPrice={gasPrice}
                setGasPrice={setGasPrice}
                amguInEth={R.path(['data', 'estimate', 'amguInEth'], estimateProps)}
                incentiveInEth={R.path(['data', 'estimate', 'incentiveInEth'], estimateProps)}
              />
