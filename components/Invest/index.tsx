...
const ParticipationFormContainer = withForm(props => <ParticipationForm {...props} />);
    ...
    <Composer
      components={[
        ...
        ({ results: [_, fundProps], render }) => {
          ...
          return <SharePriceQuery address={address} symbol={asset || firstAllowedAsset} children={render} />;
        },
        ({ results: [account], render }) => (
          <RequestQuery fundAddress={address} userAddress={account} children={render} />
        ),
      ]}
    >
      {([account, fundProps, sharePriceProps, requestProps]) => {
        const waitingTime = R.pathOr('0', ['data', 'hasActiveRequest', 'waitingTime'], requestProps);
        const hasActiveRequest = R.path(['data', 'hasActiveRequest'], requestProps);

        const isWaiting = Tm.greaterThan(waitingTime, '0');
        const readyToExecute = Tm.isZero(waitingTime) && !!hasActiveRequest;
        const isInitialRequest = Tm.isZero(R.pathOr('0', ['data', 'fund', 'totalSupply', 'quantity'], fundProps));
        const isExpired = R.path(['data', 'hasActiveRequest', 'isExpired'], requestProps);

        const sharePrice = R.path(['data', 'fund', 'sharePrice'], sharePriceProps);
        const allowedAssets = R.pathOr([], ['data', 'fund', 'investAllowed'], fundProps);
