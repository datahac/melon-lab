...
import { scan, map, combineLatest, switchMap, startWith, distinctUntilChanged } from 'rxjs/operators';
...
export const ManageTemplateContainer = ({ address, account, quoteAsset, baseAsset, fundProps, requestProps }) => {
  ...
  const allowedExchangeNames = R.pathOr([], ['data', 'fund', 'allowedExchanges'])(fundProps);

  const exchangesMap = allowedExchangeNames.map(item => [item, availableExchanges[item]]);

  const orderbookExchanges = allowedExchangeNames.filter(id => {
    return id !== 'MELON_ENGINE';
  });

  const [selectedExchanges, updateExchanges, setAllowedExchanges] = useExchangeSelector(orderbookExchanges);
  ...
  return (
    ...
    >
      {([balances, network, capability, configuration, managerProps, holdingsProps, requestProps, orderbookProps]) => {
        ...
        const isManager = !!managerProps.fund && isSameAddress(managerProps.fund, address);
        const expiredRequest = R.pathOr(false, ['data', 'hasActiveRequest', 'isExpired'], requestProps);
        ...
        return (
          <Template
            HeaderProps={{
              ...
              canonicalPriceFeedAddress: configuration && configuration.melonContracts.priceSource,
