...
const withForm = withFormik({
  ...
  validate: (values, props) => {
    ...
    } else if (Tm.greaterThan(values.quantity, props.baseToken) && values.type === 'Sell') {
    ...
    } else if (Tm.greaterThan(values.total, props.quoteToken) && values.type === 'Buy') {
      errors.total = 'Insufficient balance';
    }

    if (
      values.exchange === 'MELON_ENGINE' &&
      Tm.greaterThan(R.pathOr('0', ['total', 'quantity'], values), R.pathOr('0', ['liquidEther', 'quantity'], props))
    ) {
