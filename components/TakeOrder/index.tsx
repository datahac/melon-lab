...
import { EstimateTakeOrderMutation, ExecuteTakeOrderMutation } from '~/queries/takeOrder.gql';
    ...
    <ModalTransaction
      ...
      estimate={{
        ...
        variables: props.values && {
          ...
          buyToken: props.values.type === 'Buy' ? props.values.quantity.token.symbol : props.values.total.token.symbol,
          buyQuantity:
            props.values.type === 'Buy'
              ? props.values.quantity.quantity.toString()
              : props.values.total.quantity.toString(),
          sellToken: props.values.type === 'Buy' ? props.values.total.token.symbol : props.values.quantity.token.symbol,
          sellQuantity:
            props.values.type === 'Buy'
              ? props.values.total.quantity.toString()
              : props.values.quantity.quantity.toString(),
      ...
      execute={{
        ...
        refetchQueries: () => ['FundQuery', 'HoldingsQuery', 'OrdersQuery', 'OpenOrdersQuery'],
        onCompleted: () => {
          props.resetForm({
            price: '',
            quantity: '',
